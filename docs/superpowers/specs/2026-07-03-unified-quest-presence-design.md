...
4. **AWAY (grace):** leaving the app without locking (app switch, answered call, notification tap) starts a **30-second grace countdown** and fires a local warning notification (~12s after leaving — `WARNING_DELAY_MS`, sized above the ~10s iOS protected-data lock-signal lag so a clean lock sends nothing, and still long past any instant switch-back): *"⚠️ Your hero is in danger! Return to Emberglow or lock your phone."* On iOS the lock-state module holds a short background task (`setKeepAliveEnabled`) for the life of a presence session so the lagged lock signal is actually observed before suspension. Returning or locking rescues the quest; the away time simply doesn't count as locked time.
